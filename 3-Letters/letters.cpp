...
int kernel2dConv(image &iImg,int iCol,int iRow,int **ker,image &k){
...
unsigned char *convolveSimple(image &iImg,image &k,int **kMSF_t){
...
int **computeMSFMeanKernel(image &ker){
	
	unsigned int kerMean = 0;
	unsigned char **ppkPixels = ker.getppPixels();
...
unsigned char *getMSFImage(image &ker,image &iImg,int iT){
	...
	char msfImg[30];
	strcpy(msfImg,"img/msf_e.ppm");
	saveImage(pMSFImage,iImg.getCols(),iImg.getRows(),msfImg);
...
void TestMSFImage(unsigned char *pMSFImage,char infileName[30],image &img,image &ker){
...
int main(int argc, char *argv[]){
  ...
  saveImage(pMSFImage,img.getCols(),img.getRows(),fileName);
